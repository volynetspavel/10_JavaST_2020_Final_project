...
public class TestConnectionPool {
	...
	private BlockingQueue<TestPooledConnection> availableConnections;
	private Set<TestPooledConnection> usedConnections;

	static {
		MAX_POOL_SIZE = TestConnectorDB.obtainMaxPoolSize();
		CONNECTION_URL = TestConnectorDB.obtainConnectionURL();
	}

	private TestConnectionPool() throws ConnectionPoolException {
		try {
			Class.forName(TestConnectorDB.obtainDriver()); // register driver
	...
	private void init() throws ConnectionPoolException {
		...
		for (int i = 0; i < MAX_POOL_SIZE; i++) {
			try {
				availableConnections.put(new TestPooledConnection(
						DriverManager.getConnection(CONNECTION_URL, TestConnectorDB.obtainProperties())));
	...
	public Connection takeConnection() {
		...
		TestPooledConnection connection = null;
		...
		try {
			...
			LOGGER.debug("================= Take test connection =================");
	...
	public void returnConnection(Connection connection) {
		try {
			if (connection instanceof TestPooledConnection) {
				TestPooledConnection proxyConnection = (TestPooledConnection) connection;
	...
	public void closePool() throws ConnectionPoolException {
		try {
			...
			for (int i = 0; i < MAX_POOL_SIZE; i++) {
				...
				try {
					TestPooledConnection connection = availableConnections.take();
